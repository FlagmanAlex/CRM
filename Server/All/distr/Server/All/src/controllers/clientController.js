...
const deleteClient = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const clientId = req.params.id;
        const respOrder = yield order_model_1.Order.find({ clientId: clientId });
        if (respOrder.length === 0) {
            ...
            res.status(503).json({ message: `Нельзя удалить этого клиента.\nКлиент присутствует в ${respOrder.length} операциях \nСначала удалите все документы с этим клиентом.` });
        }
    }
    catch (error) {
        console.log('Ошибка базы данных', error);
